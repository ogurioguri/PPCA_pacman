    ...
    def train(self, dataset):
        ...
        with no_grad():
            ...
            for batch in dataloader:
                prediction = self.get_prediction(batch['x'])
                if prediction != batch['label']:
                    self.w += batch['label'] * batch['x']
                    flag = False
...
class RegressionModel(Module):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__()
        self.layer1 = Linear(1, 300)
        self.layer2 = Linear(300, 300)
        self.layer3 = Linear(300, 1)
...
class LanguageIDModel(Module):
    ...
    def run(self, xs):
        ...
        "*** YOUR CODE HERE ***"
        x = tensor(())
        x = relu(self.layer1(xs[0]))
        for i in range(1,len(xs)):
            x = relu(self.layer1(xs[i]) + self.layer2(x))
        x = self.layer3(x)
        return x
    ...
    def train(self, dataset):
        ...
        for i in range(10):
            for batch in dataloader:
                optimizer.zero_grad()
                loss = self.get_loss(movedim(batch['x'], 0, 1), batch['label'])
                loss.backward()
                optimizer.step()
...
def Convolve(input: tensor, weight: tensor):
    ...
    for i in range(output_height):
        for j in range(output_width):
            Output_Tensor[i,j] = tensordot(input[i:i+kH, j:j+kW], weight)
...
class DigitConvolutionalModel(Module):
    ...
    def __init__(self):
        ...
        """ YOUR CODE HERE """
        self.layer1 = Linear(676, 300)
        self.layer2 = Linear(300, 300)
        self.layer3 = Linear(300, output_size)
