    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # get the most likely position of each ghost
        ghostPositions = [max(belief, key=belief.get) for belief in livingGhostPositionDistributions]
        # get the closest ghost
        closestGhost = min(ghostPositions, key=lambda x: self.distancer.getDistance(pacmanPosition, x))
        # get the closest action to the closest ghost
        closestAction = min(legal, key=lambda x: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, x), closestGhost))
