...
def joinFactors(factors: List[Factor]):
    ...
    domain = (factors_[0]).variableDomainsDict()
    #domain = {}
    unconditioned = set()
    conditioned = set()
    ...
    result = Factor(list(unconditioned),list(conditioned),domain)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        unconditioned = set(factor.unconditionedVariables()) - set([eliminationVariable])
        conditioned = set(factor.conditionedVariables()) - set([eliminationVariable])
        new = Factor(list(unconditioned),list(conditioned),domain)
        for assignment in new.getAllPossibleAssignmentDicts():
            prob = 0
            for value in domain[eliminationVariable]:
