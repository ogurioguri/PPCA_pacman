...
def constructBayesNet(gameState: hunters.GameState):
    ...
    #最大的范围差别
    variableDomainsDict[OBS0] = list(range((X_RANGE + Y_RANGE - 1) + MAX_NOISE))
    variableDomainsDict[OBS1] = list(range((X_RANGE + Y_RANGE - 1) + MAX_NOISE))
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        #result = Factor(queryVariables, evidenceDict.keys(), bayesNet.variableDomainsDict())
        ...
        for joinVariable in eliminationOrder:
            ...
            #如果joinedFactor只有一个unconditionedVariable，那么不需要eliminate
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        truedistacne = manhattanDistance(pacmanPosition, ghostPosition)
        ...
        if ghostPosition == jailPosition:
            return 0
        return busters.getObservationProbability(noisyDistance, truedistacne)
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldposition in self.allPositions:
            newdistribution[oldposition] = self.getPositionDistribution(gameState, oldposition)
            result[oldposition] = 0
        for oldposition in self.allPositions:
            for newposition in self.allPositions:
                result[newposition] += newdistribution[oldposition][newposition] * self.beliefs[oldposition]
        for oldposition in self.allPositions:
            self.beliefs[oldposition] = result[oldposition]
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        newlist = []
        for particle in self.particles:
            belief[particle] += self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
        if(belief.total() == 0):
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for oldposition in self.particles:
            ...
            result[oldposition] = 0
        for oldposition in self.particles:
            for newposition in self.legalPositions:
                result[newposition] += newdistribution[oldposition][newposition]
        if result.total() == 0:
            self.initializeUniformly(gameState)
            result = self.getBeliefDistribution()
        newlist = []
        for i in range(self.numParticles):
            newlist.append(result.sample())
        self.particles = newlist
        return newlist
        "*** END YOUR CODE HERE ***"
